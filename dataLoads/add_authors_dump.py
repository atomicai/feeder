...
if not rdb.db('meetingsBook').table_list().contains('authors').run(conn):
    rdb.db('meetingsBook').table_create('authors',primary_key='idx').run(conn)

for row in arr:
    id = uuid.uuid4()
    rdb.db('meetingsBook').table('authors').insert({'name': row["name"],'id': str(id)}).run(conn)
